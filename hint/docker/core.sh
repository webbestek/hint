...
printf "DOCKER	Compose up (bg)	docker compose up -d	Start services in background\n"
printf "DOCKER	Compose down	docker compose down	Stop and remove services\n"
printf "DOCKER	Logs (follow)	docker compose logs -f --tail=100	Follow multi-service logs\n"
printf "DOCKER	Exec shell	docker compose exec <service> bash	Open a shell in container\n"
