...
if command -v fpm >/dev/null 2>&1; then
  ...
  pushd "$WORK" >/dev/null
  fpm -s dir -t deb -n hint -v "$VERSION"     --license MIT     --description "Starship-powered interactive command palette"     --vendor "Kevin Ahrens"     --url "https://github.com/YOUR-ORG/hint"     --after-install "$ROOT/scripts/install.sh"     usr/local/share/hint usr/local/bin/hint usr/local/bin/sysline -p "$DIST"
  popd >/dev/null
